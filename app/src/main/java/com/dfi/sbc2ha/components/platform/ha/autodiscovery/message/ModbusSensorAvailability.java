...
public class ModbusSensorAvailability extends IterableAvailability {
    ...
    public ModbusSensorAvailability(ModbusSensorConfig sensorConfig, ModbusSensorDefinition def) {
        ...
        getAvailability().get(0).put(TOPIC, formatTopic(topicPrefix, deviceId + STATE));


        setUniqueId(topicPrefix + getId().toLowerCase() + sensorConfig.getName().toLowerCase());
    }
    ...
    public ModbusSensorAvailability next() {
        ...
        setUniqueId(topicPrefix + getStateDeviceType().toLowerCase() + deviceId + getId());
    ...
    public String getStateTopic(Register register) {
        return formatTopic(topicPrefix, getStateDeviceTypeName(), deviceId, register.getName());
    }
}
