...
*/
    ...
    public void handleClick(StateEvent<ButtonState> evt) {
        if (!started) {
            drawBoot();
        }
        stopSleepHandler();
    ...
    private void renderScreen() {
        ...
        switch (currentScreen) {
            ...
            case OUTPUTS:
                ...
                Map<String, String> outputStates = outputDataProvider.getLines();
    ...
    private void drawStandard(List<String> list) {
        ...
        int line = 15;
        int step = 10;
        g2d.drawString(list.get(0), 3, line);
        g2d.setFont(fontSmall);

        for (var i = 1; i < list.size(); i++) {
            line += step;
            g2d.drawString(list.get(i), 3, line);
        }
        displayDelegate.display(image);
    }

    private void drawOutputs(Map<String, String> outputStates) {
        Logger.debug("display outputs");
        ...
        try {
            ...
            Set<Map.Entry<String, String>> entries = outputStates.entrySet();
            List<Set<Map.Entry<String, String>>> lists = DataUtil.split(entries, 6);

            for (Map.Entry<String, String> entry : lists.get(0)) {
            ...
            if (lists.size() > 1) {
                line = 0;
                for (Map.Entry<String, String> entry : lists.get(1)) {
                    String i = entry.getKey();
                    String s = entry.getValue();
                    line += 10;
    ...
    private void drawSwap(List<String> lines) {
        Logger.debug("display swap");
        drawStandard(lines);
    }

    private void drawMemory(List<String> lines) {
        Logger.debug("display memory");
        drawStandard(lines);
    }

    private void drawDisk(List<String> lines) {
        Logger.debug("display disk");
        drawStandard(lines);
    }

    private void drawCpu(List<String> lines) {
        Logger.debug("display cpu:");
        drawStandard(lines);
    }

    private void drawNetwork(List<String> lines) {
        Logger.debug("display network");
        drawStandard(lines);
    }

    private void drawUptime(List<String> lines) {
        Logger.debug("display uptime");
        ...
        try {
            g2d.drawString(lines.get(0), 3, 42);
            g2d.drawString(lines.get(1), 3, 54);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        displayDelegate.display(image);
