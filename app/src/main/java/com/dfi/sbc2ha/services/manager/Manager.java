...
import com.dfi.sbc2ha.components.actuator.*;
import com.dfi.sbc2ha.components.platform.bus.Bus;
import com.dfi.sbc2ha.components.platform.bus.BusFactory;
import com.dfi.sbc2ha.components.platform.ha.autodiscovery.SbcDeviceType;
import com.dfi.sbc2ha.components.platform.ha.autodiscovery.message.*;
import com.dfi.sbc2ha.components.platform.ha.command.LightCommand;
import com.dfi.sbc2ha.components.platform.mqtt.MqttPlatform;
import com.dfi.sbc2ha.components.platform.oled.Display;
import com.dfi.sbc2ha.components.platform.oled.DisplayFactory;
import com.dfi.sbc2ha.components.sensor.BinarySensor;
import com.dfi.sbc2ha.components.sensor.ScalarSensor;
import com.dfi.sbc2ha.components.sensor.Sensor;
import com.dfi.sbc2ha.components.sensor.SensorFactory;
import com.dfi.sbc2ha.components.sensor.binary.Binary;
...
import static com.dfi.sbc2ha.components.platform.mqtt.hivemq.MqttHelperHiveRx.payloadToString;
...
public class Manager implements AutoCloseable {
    ...
    private Map<String, Actuator> actuatorMap = new HashMap<>();
    private List<Bus<?>> busMap = new ArrayList<>();
    ...
    private void registerSensors() {
        ...
        for (SensorConfig sensorConfig : config.getSensor()) {
            try {
                ...
                if (sensor.listenersEmpty()) {
                    sensor.whenAny(stateEvent -> {
    ...
    public void command(StateCommand cmd) {
        if (cmd instanceof ActuatorCommand) {
        ...
        } else if (cmd instanceof SensorCommand) {
            Sensor sensor = sensorMap.get(((SensorCommand) cmd).getSensor());
            if (sensor != null) {
                if (cmd instanceof BinaryCommand && sensor instanceof Binary) {
                    ((Binary) sensor).handleFakeState(((BinaryCommand) cmd).getState());
                } else if (cmd instanceof ButtonCommand && sensor instanceof Button) {
                    ((Button) sensor).handleFakeState(((ButtonCommand) cmd).getState());
                }
            }
        }
    }
    ...
    public void close() {
        ...
        sensorMap.clear();
        sensorMap = null;
        actuatorMap.forEach((s, actuator) -> {
            actuator.close();
        });
        actuatorMap.clear();
        actuatorMap = null;
        ...
        if (actuatorMap.containsKey(name)) {
            ...
            if (actuator instanceof TextField) {
                if (action.contains("=")) {
                    ...
                    var level = LogLevelType.valueOf(split[1].trim().toUpperCase());

                    LoggerConfig configLogger = config.getLogger();
                    configLogger.getLogs().put(log, level);

                    LogUtil.configureLogging(configLogger);
    ...
    public Set<InitialState> getCurrentState() {
        Set<InitialState> all = new HashSet<>();
        synchronized (actuatorMap) {
            synchronized (sensorMap) {
                sensorMap.forEach((s, sensor) -> {
                    if (sensor instanceof ScalarSensor) {
                        float value;
                        try {
                            value = ((ScalarSensor) sensor).getValue();
                        } catch (RuntimeException e) {
                            log.error("get initial valu for sensor:{} failed", sensor.getName());
                            value = 0;
                        }
                        all.add(new InitialState("sensor", sensor.getName(), String.valueOf(value)));
                    }
                });
                actuatorMap.forEach((s, actuator) -> {
                    all.add(new InitialState("actuator", String.valueOf(actuator.getId()), actuator.getValue()));
                });
            }
        }
