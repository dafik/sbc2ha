...
@Component({
  selector: 'app-actuator-mcp',
  templateUrl: './actuator-mcp.component.html',
  styleUrls: ['./actuator-mcp.component.scss']
})
export class ActuatorMcpComponent {
  ...
  constructor(private _formBuilder: FormBuilder,
              public dialogRef: MatDialogRef<ActuatorMcpComponent>,
              @Inject(MAT_DIALOG_DATA) public data: { config: McpOutputConfig },
  ...
  add() {
    ...
    let config: McpOutputConfig = new McpOutputConfig(value.name as string, value.outputType as ActuatorType, Number(value.output))
  ...
  edit() {
    let value = this.platform.value;
    let config: McpOutputConfig = this.data.config;
