...
@Component({
  selector: 'app-actuator-pca',
  templateUrl: './actuator-pca.component.html',
  styleUrls: ['./actuator-pca.component.scss']
})
export class ActuatorPcaComponent {
  ...
  constructor(private _formBuilder: FormBuilder,
              public dialogRef: MatDialogRef<ActuatorPcaComponent>,
              @Inject(MAT_DIALOG_DATA) public data: { config: PcaOutputConfig },
  ...
  add() {
    ...
    let config: PcaOutputConfig = new PcaOutputConfig(value.name as string, value.outputType as ActuatorType, Number(value.output))
  ...
  edit() {
    let value = this.platform.value;
    let config: PcaOutputConfig = this.data.config;
